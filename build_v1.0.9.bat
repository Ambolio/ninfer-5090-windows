@echo off
rem ============================================================
rem NInfer v1.0.9 (upstream b88c0f6f + fork ports 2026-09-09 +
rem make_prefill_work fix + Windows test ports + packed-verify
rem C=1 route pin 2026-09-10) - RTX 5090 (sm_120a), vision.
rem Autocontenido: solo necesita este arbol + MSVC BuildTools +
rem CUDA 13.3 + Ninja (los tres ya instalados; rutas en el script).
rem
rem Uso:
rem   build_v1.0.9.bat              -> build en _build_5090new (relativo al arbol)
rem   build_v1.0.9.bat <build_dir>  -> build en el directorio indicado
rem Sin "pause": apto para correr en background y redirigir log.
rem ============================================================
setlocal
set BUILD_DIR=%~1
if "%BUILD_DIR%"=="" set BUILD_DIR=%~dp0\_build_5090new
cd /d "%~dp0"
...
echo [build] dir=%BUILD_DIR% arch=120a
cmake -B "%BUILD_DIR%" -S . -G Ninja -DCMAKE_CUDA_ARCHITECTURES="120a" -DNINFER_ENABLE_AVX2=ON -DNINFER_BUILD_MEDIA_ACQUIRE=ON -DCMAKE_BUILD_TYPE=Release
