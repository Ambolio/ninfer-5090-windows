...
struct Variant {
    ...
    // route_tokens is the C=1 width of a packed verify round (0 when not batched packed verify).
    // The 35B-A3B package is all-A16: every route below is T-family-stable, so the parameter is
    // accepted for the family interface and ignored.
